...
- Gemma's tools, called with a line like `TOOL: contact Atiq`: `time`, `remember <fact>`, `open <app>`, `contact <name>`, `call <name|number>`, `whatsapp <name>: <text>`, `sms <name>: <text>`, `location`, `battery`, `bluetooth` (status) / `bluetooth on|off`, `wifi on|off`, `flashlight on|off`, `alarm <time>`, `timer <duration>`, `search <words>`. Readers are `tools/LocationTool`, `BatteryTool`, `BluetoothTool`; actions are `workflow/PhoneActions`; alarm/timer text is read by `agent/TimeSpecs`. Safety: `call` opens the dialer (user taps call), WhatsApp/SMS open prefilled (user taps Send); Android won't let apps toggle Wi-Fi/Bluetooth off, so those open the system switch. `agent/ToolCalls.kt` reads them (tolerant of markdown, `ACTION:`, `<>`, "'s number"); `workflow/WorkflowEngine.kt` runs them. Tool results are shown directly, not sent back to Gemma (one pass, and Gemma can't ignore the data).
