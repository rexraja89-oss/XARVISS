...
- Gemma has four tools, called with a line like `TOOL: contact Atiq`: `time`, `remember <fact>`, `open <app>`, `contact <name>`. `agent/ToolCalls.kt` reads them (tolerant of markdown, `ACTION:`, `<>`, "'s number"); `workflow/WorkflowEngine.kt` runs them. Tool results are shown directly, not sent back to Gemma (one pass, and Gemma can't ignore the data).
