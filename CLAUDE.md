...
- The screen shows `B<build number>` (GitHub run number = versionCode) at the start of the AI MODEL line, to confirm which build is installed. When a fix "doesn't work", first check the phone's B number: twice Rex's S22 was still on an old build (the chat history surviving is another sign; an update restarts XARVIS and clears it).
