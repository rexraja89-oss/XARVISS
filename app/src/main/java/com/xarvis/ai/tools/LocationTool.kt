...
class LocationTool(context: Context) : DeviceTool {
    ...
    private companion object {
        ...
        /** English and Hindi/Urdu ways of asking where the phone is. */
        val TOPIC = Regex(
            """\b(location|locate|where am i|where i am|where are we|my position|current position|address|""" +
                """kaha+n|kaha\s+(?:hoo?n?|hu+n?|hai|hain)|kidh?ar)\b"""
        )

        /** "address" that isn't about a place. */
        val NOT_A_PLACE = Regex("""\b(email|e-mail|ip|mac|web|website)\s+address\b""")
    }
}
