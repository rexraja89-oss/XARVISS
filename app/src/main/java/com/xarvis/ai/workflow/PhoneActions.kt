...
/**
 * Things XARVIS does on the phone: calls, WhatsApp and SMS, flashlight, alarms and timers,
 * Bluetooth and Wi-Fi. Messages are opened ready to send rather than sent, so the user
 * always taps Send themselves.
 */
class PhoneActions(context: Context) {
    ...
    /** [direct] places the call at once; otherwise the dialer opens with the number ready. */
    suspend fun call(target: String, direct: Boolean): StepResult {
        ...
        return start(
            ...
            "Opened the dialer for ${who.display}. Tap the call button to call.",
        )
    }
